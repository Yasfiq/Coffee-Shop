...
const Menu = (props) => {
          ...
          <div className="space-x-3">
            ...
            <button className="p-3 rounded-full bg-red-500 relative">
              <img
                ...
                width="30"
                onClick={() => {
                  handledeleteProduct(dataProduct.productname,dataProduct.id);
                }}
              />
            </button>
          </div>
  ...
  }, [dispatch, props.category, props.order, props.search, props.limit]);

  useEffect(() => {
    if (deleteResult || addResult) {
      dispatch(getProduct(props.category));
    }
  }, [dispatch, props.category, deleteResult, addResult]);
