            ...
            <form className="container mx-auto font-rubik grid grid-cols-12 py-10" onSubmit={handleSubmit} encType="multipart/form-data">
                ...
                <div className="col-span-4 flex flex-col p-10 pt-16">
                    <div className="w-72 h-72 flex justify-center items-center overflow-hidden mx-auto bg-[#bababa40] rounded-full mb-10">
                        <img src={userInfo.filepreview !== null ? userInfo.filepreview : product[0] ? `http://localhost:3000/uploads/${product[0].productimage[0].filename}` : cameraIcon} alt="camera-icon" width="100%" />
                    </div>
                    <div className="w-full p-3 grid grid-cols-3 gap-3 items-center rounded-xl bg-[#f4f4f4]">
                        ...
                        }) : product[0] ? product[0].productimage.map(image => {
                            return <>
                                <img src={`http://localhost:3000/uploads/${image.filename}`} alt={`${image.name}`} className="block h-full mr-3" />
                            </>
                        }) : ''}
                    </div>
                    <input className="hidden" type="file" name="productimage" id="productimage" onChange={handleInputChange} multiple/>
                    <label for="productimage" className="btn-primary hover:shadow-xl hover:shadow-primary/50 duration-500 self-center cursor-pointer text-xl py-5" >Choose from gallery
                    </label>
                ...
                <div className="col-span-8 px-20 py-10">
                    ...
                    <input type="text" id="productname" name="productname" className="input-oneline text-base mb-10" placeholder="Type product name min.3 characters" defaultValue={`${(product[0]) ? product[0].productname : ''}`}/>
                    {/*  */}
                    <label for="price" className="font-bold text-secondary text-xl">Price :</label>
                    <input type="text" id="price" name="price" className="input-oneline text-base mb-10" placeholder="Type the price" defaultValue={`${(product[0]) ? product[0].price : ''}`}/>
                    {/*  */}
                    <label for="category" className="font-bold text-secondary text-xl">Category :</label>
                    <input type="text" id="category" name="category" className="input-oneline text-base mb-10" placeholder="Type the category" defaultValue={`${(product[0] && product[0].category !== 'undefined') ? product[0].category : ''}`}/>
                    {/*  */}
                    <label for="description" className="font-bold text-secondary text-xl">Description :</label>
                    <input type="text" id="description" name="description" className="input-oneline text-base mb-10" placeholder="Describe your product min. 100 characters" defaultValue={`${(product[0] && product[0].description !== undefined) ? product[0].description : ''}`} />
